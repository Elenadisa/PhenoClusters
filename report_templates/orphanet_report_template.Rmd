...
```{r  echo=FALSE, warning=FALSE, message=FALSE, results='asis'}
  ...
  kegg_metric_table <- read.table("../results/kegg/orpha_kegg_comention_table_metrics.txt", sep ="\t", header = TRUE, stringsAsFactors = FALSE, quote = "")
  go_metric_table <- read.table("../results/go/orpha_go_comention_table_metrics.txt", sep ="\t", header = TRUE, stringsAsFactors = FALSE, quote = "")
  reactome_metric_table <- read.table("../results/reactome/orpha_reactome_comention_table_metrics.txt", sep ="\t", header = TRUE, stringsAsFactors = FALSE, quote = "")
...
group_cluster_information <- function(df1, df2){
	...
	for (i in clusters){
		...
		if (nrow(system_groups) != 0) {
			system_groups_sorted <- system_groups[order(-system_groups$Phenotype_percentage),]
      groups[["system"]] <- system_groups_sorted
			scores <- system_groups$Phenotype_percentage
          	score <- max(scores)
    ...
    if(length(funsys_l) > 1){
      ...
      for(parent in parents_l){
        if(parent %in% funsys_l){
          cluster_subset <- (cluster_subset[!cluster_subset$Funsys == parent, ])
        }
      }
...
```{r  echo=FALSE, warning=FALSE, message=FALSE}
  summary_metric_table <- read.table("../results/orphanet_summary", sep="\t", header = FALSE)
...
```{r  echo=FALSE, warning=FALSE, message=FALSE}
  pairs_metric_table <- read.table("../results/orphanet_pairs_table", sep="\t", header = FALSE)
...
```{r  echo=FALSE, warning=FALSE, message=FALSE}
produce_barplot("pairs_number", "Significant Pairs Number Orphanet", "Number of pairs")
...
In this step we will get phenotype-genes pairs. Through a functional assignment of these genes we can get phenotype-function pairs.
 ...
 ```{r  echo=FALSE, warning=FALSE, message=FALSE}
  gene_metric_table <- read.table("../results/orphanet_gene_table", sep="\t", header = FALSE)
...
kable(ddply(kegg_metric_table, .(Type), summarize, "Co-occurrent pairs" = paste(round(mean(number_coocurrent_pairs_kegg),3), "\U00B1" , round(sd(number_coocurrent_pairs_kegg),3)), "No co-occurrent pairs" = paste(round(mean(number_non_coocurrent_pairs_kegg),3), "\U00B1" , round(sd(number_non_coocurrent_pairs_kegg),3))))
...
kable(ddply(go_metric_table, .(Type), summarize, "Co-occurrent pairs" = paste(round(mean(number_coocurrent_pairs_go),3), "\U00B1" , round(sd(number_coocurrent_pairs_go),3)), "No co-occurrent pairs" = paste(round(mean(number_non_coocurrent_pairs_go),3), "\U00B1" , round(sd(number_non_coocurrent_pairs_go),3))))
...
grid.arrange(
  produce_barplot("Cluster_number", "Cluster Number Orphanet", "Number of clusters"),
  produce_barplot("Average_Cluster_size", "Cluster Size Orphanet", "Average Cluster size"),
  ncol = 2
)
...
kable(ddply(metric_table, .(Type), summarize, "Cluster Number" = paste(round(mean(Cluster_number),3), "\U00B1" , round(sd(Cluster_number),3)), "Average Cluster size" = paste(round(mean(Average_Cluster_size),3), "\U00B1" , round(sd(Average_Cluster_size),3))))
...
kable(ddply(metric_table, .(Type), summarize, "Cluster with 70 coherence_0.05" = paste(round(mean(Cluster_with_70_coherence_0.05),3), "\U00B1" , round(sd(Cluster_with_70_coherence_0.05),3)), "Cluster with 50 coherence_0.05" = paste(round(mean(Cluster_with_50_coherence_0.05),3), "\U00B1" , round(sd(Cluster_with_50_coherence_0.05),3))))  
```
...
clusters_hpo <- read.table("PATH/TO/FILES/neuromuscular_diseases_project/orpha_analysed_networks/orpha_neuromuscular_pairs/clustering.R_0000/clusters_file_with_names", sep ="\t", quote="", stringsAsFactors = FALSE, header = TRUE)
...
kegg <- read.table("PATH/TO/FILES/neuromuscular_diseases_project/orpha_analysed_networks/orpha_neuromuscular_pairs/clusters_single_funsys_merger.py_0000/clusters_with_70_coherence_systems_kegg_0.05", sep ="\t", quote="", stringsAsFactors = FALSE, header = FALSE)
colnames(kegg) <- c("Cluster", "Funsys", "Name", "Phenotype_percentage", "Genes")
if(nrow(kegg) > 0){
    for (i in 1:length(kegg$Funsys)){
      url <- paste0("https://www.genome.jp/dbget-bin/www_bget?pathway+", kegg$Funsys[i])
      kegg$Funsys[i] <- text_spec(kegg$Funsys[i], link = url)
    }
  }

go_df <- read.table("PATH/TO/FILES/neuromuscular_diseases_project/orpha_analysed_networks/orpha_neuromuscular_pairs/clusters_single_funsys_merger.py_0001/clusters_with_70_coherence_systems_go_0.05", sep ="\t", quote="", stringsAsFactors = FALSE, header = FALSE)
colnames(go_df) <- c("Cluster", "Funsys", "Name", "Phenotype_percentage", "Genes")
go <- delete_parental_terms(go_df)
if(nrow(go) > 0){  
    for (i in 1:length(go$Funsys)){
      url <- paste0("http://amigo.geneontology.org/amigo/term/", go$Funsys[i])
      go$Funsys[i] <- text_spec(go$Funsys[i], link = url)
    }
  }

reactome <- read.table("PATH/TO/FILES/neuromuscular_diseases_project/orpha_analysed_networks/orpha_neuromuscular_pairs/clusters_single_funsys_merger.py_0002/clusters_with_70_coherence_systems_reactome_0.05", sep ="\t", quote="", stringsAsFactors = FALSE, header = FALSE)
colnames(reactome) <- c("Cluster", "Funsys", "Name", "Phenotype_percentage", "Genes")
if(nrow(reactome) > 0){  
    for (i in 1:length(reactome$Funsys)){
      url <- paste0("https://reactome.org/content/detail/", reactome$Funsys[i])
      reactome$Funsys[i] <- text_spec(reactome$Funsys[i], link = url)
    }
  }
...
#clusters_file
clusters_file_cytoscape <- read.table("PATH/TO/FILES/neuromuscular_diseases_project/orpha_analysed_networks/orpha_neuromuscular_pairs/clustering.R_0000/cytoscape_file", sep=" ", header=FALSE, quote="", stringsAsFactors = FALSE)
